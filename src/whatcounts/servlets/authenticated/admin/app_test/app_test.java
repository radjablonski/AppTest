package whatcounts.servlets.authenticated.admin.app_test;


import artamedia.registration.*;
import whatcounts.BaseServletIO;
import whatcounts.admin.tests.apptestgateway.AppTest;
...
public class app_test extends BaseServletLoginRequired {
	...
	public void process(BaseServletIO bio, User u)	{
		...
		date = new Date();    
		System.out.println("Process starting time - " + dateFormat.format(date));
		...
		date = new Date(); 
		System.out.println("Process ending time - " + dateFormat.format(date));
	}
}
